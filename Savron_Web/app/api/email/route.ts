...
function getIcsString(booking: any, barberName: string): string {
...
export async function POST(request: NextRequest) {
  ...
  const barberName = (booking.barbers as { name: string } | null)?.name ?? booking.barber_name ?? 'Your barber';
...
<body style="margin:0;padding:0;background:#050505;font-family:Arial,sans-serif;">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#050505;border:1px solid rgba(255,255,255,0.08);margin-bottom:28px;">
              ...
              <tr>
                <td style="padding:14px 20px;">
                  ...
                  <span style="color:#4ade80;font-size:18px;font-weight:700;">${booking.price}</span>
                </td>
              </tr>` : ''}
            </table>

            <p style="margin:0 0 6px;color:rgba(255,255,255,0.4);font-size:12px;line-height:1.6;">
              Need to cancel or reschedule? Reply to this email or call us at <a href="tel:+16125550100" style="color:#4ade80;">612-555-0100</a>.
            </p>
  ...
  const icsString = getIcsString(booking, barberName);
  
  // MOCK .pkpass Generation (Pending Apple Developer Certificates)
  // When active Apple Developer certificates are available, replace this with `passkit-generator`
  const pkpassBuffer: string | null = null; 
  ...
  if (pkpassBuffer) {
      attachments.push({
          filename: 'appointment.pkpass',
          content: pkpassBuffer, // base64 payload
      });
  }
